 ...
 *
 * Copy `.env.example` to `.env` (at the app root, i.e. `api/.env` — see
 * CakePHPDivergance.md) and set/modify the variables as required.
 *
 ...
 *
 * There is no `config/app_local.php` here — see CakePHPDivergance.md. All
 * per-environment values come from `.env` (loaded above) via `env()` calls in
 * app.php itself.
 */
try {
 ...
 *
 * Set APP_FULL_BASE_URL in `.env` (see api/.env.example) or configure App.fullBaseUrl
