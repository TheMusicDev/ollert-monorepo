...
`ci.yml` runs three independent jobs in parallel on every PR against `main`: `api-tests`, `web-tests`, `pr-agent`. `api-tests`/`web-tests` need no setup — they run out of the box. `pr-agent` needs one manual step below before it can run.
