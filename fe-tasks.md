...
3. **Ownership/authorization fields the FE needs must be nailed down in `api-contract.md` before Section 2 starts, not guessed per-branch.** No `is_owner` (or equivalent) field is documented for the org resource, and there's no `/me` endpoint — so the FE has no way to resolve "is the current user this org's owner" without guessing. `feat/web-orgs`, `feat/web-org-members`, and `feat/web-boards` each shipped a different guess. **Fix pattern**: this is a cross-cutting BE/FE contract decision — see `be-tasks.md` Section 1F, which now has an explicit item to expose this. Once the BE branch that builds `OrganizationsController` ships, reconcile all three FE branches against the real shape in one pass instead of leaving three divergent guesses live.
