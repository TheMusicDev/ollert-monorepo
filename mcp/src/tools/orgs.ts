...
export function registerOrgs(server: McpServer, bearer: string): void {
  server.registerTool(
    ...
    async (a) => run(() => apiFetch<Paginated<Org>>(`/api/orgs${pagination(a)}`, bearer)),
  );
  ...
  server.registerTool(
    "get_org",
    {
      description: "Get one org with its boards.",
  ...
  server.registerTool(
    "delete_org",
    {
      description: "Delete an org (owner only; soft delete).",
      inputSchema: z.object({ id: z.string().uuid() }),
