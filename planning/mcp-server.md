...
**Open risk, confirmed 2026-08-24**: the Supabase project's native OAuth server is **disabled** — `GET .../.well-known/oauth-authorization-server/auth/v1` returns `404 {"error_code":"feature_disabled","msg":"OAuth server is disabled"}`. This is the real blocker for the end-to-end claude.ai↔Supabase OAuth dance (authorize/token/DCR all live behind that feature flag) and must be enabled in the Supabase dashboard before any hosted-connector flow can run. The `mcp/` Resource Server itself is built and boots fine without it — it only fetches that AS-metadata doc once at startup to build the RFC 9728 `authorization_servers` pointer, so a disabled AS metadata endpoint makes `buildHandler()` exit(1) at boot. Local boot was verified against a stub AS-metadata doc (see [Change log](log.md) 2026-08-24).
...
Kamal deploys a Docker container to a machine on the local network; Cloudflare (Tunnel) exposes it publicly. Chosen because the existing shared-PHP-host deploy target (see [Architecture § Deployment](architecture.md#deployment)) has no Node runtime — `mcp/` needs its own target. OAuth testing is against the real hosted Supabase project (no local Supabase CLI stack in this repo — see [Architecture § Local Development](architecture.md#local-development)); blocked until the Supabase OAuth server is enabled (see Open risks).
...
* `aud`-claim compatibility between OAuth-flow tokens and `SUPABASE_JWT_AUD` (`authenticated`) — unverified until a real claude.ai dance runs (server now on, so testable).
