...
* **DCR — ENABLED 2026-09-07** (reverses the original "DCR left off" call from this same day in 2026-08-24). Supabase Cloud added an "Allow Dynamic OAuth Apps" dashboard toggle (Authentication → OAuth Server) that wasn't present/documented when the off-by-design decision was made. Flipped on after real friction: `claude mcp add` failed outright ("Incompatible auth server: does not support dynamic client registration") without a manually pre-registered `client_id`. `GET …/.well-known/oauth-authorization-server` now advertises a `registration_endpoint`; verified end-to-end with a flagless `claude mcp add --transport http ollert <url>` → OAuth flow → tool call. The original tradeoff (self-registered clients can pick any name/logo shown on Ollert's own `/oauth/consent` screen) is accepted, not eliminated — DCR skips pre-registration, not the per-connection approval the account owner still does on that screen. See [Change log](log.md) 2026-09-07.
