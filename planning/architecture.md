...
* **Frontend**: TanStack Start, React, TypeScript, Tailwind CSS, Base UI (unstyled/headless component primitives), Bun as package manager and script runner. Supabase JS client (`@supabase/supabase-js`) for auth only — no Supabase DB/Realtime client calls, since app data doesn't live there.
