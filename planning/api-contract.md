...
* `PATCH /api/admin/users/:id` - set per-user quota overrides (`max_orgs`/`max_boards_per_org`/`max_lists_per_board`/`max_cards_per_board`). Overrides are the only mechanism to raise a limit above the floor — no global raise (see [Data Model](data-model.md#quotas)). Whether `is_admin` is patchable here (promote/demote other admins) or env-`ADMIN_UUID`-bootstrap-only is TBD when the implementing branch builds it.

Contract details (exact patchable field set, whether partial PATCH is allowed, pagination of the user list) finalized when the admin branch implements this — same TBD-then-settled pattern `is_owner` used (see [log.md](log.md)).
