...
* **DB**: MySQL owned by CakePHP, not Supabase Postgres — keeps Supabase strictly to identity, avoids coupling app schema to a Supabase project. ~~**SUPERSEDED 2026-08-24**~~ by the all-in-Supabase decision below — kept here as the historical record; both local dev and prod (negrita) run Postgres now (spine landed 2026-08-27, prod cutover complete 2026-08-30).
...
* **Local dev DB**: originally Docker Compose (`docker/`) running MariaDB + Mailpit. ~~**SUPERSEDED 2026-08-27**~~ — replaced by the Supabase CLI local stack (`supabase start`) as part of the all-in-Supabase migration; `docker/` deleted 2026-08-29. See [Architecture](architecture.md#local-development).
...
* **All-in Supabase (2026-08-24, reverses the DB decision above)**: app data moves from CakePHP-owned MySQL to Supabase Postgres, with Storage + Realtime adopted alongside the existing Auth. Major overhaul. **2026-08-27: spine landed locally** — CakePHP runs against the Supabase CLI local stack (`supabase start`, both Postgres and local Auth/JWKS) end-to-end. **2026-08-30: prod cutover complete** — negrita's API redeployed against the same hosted Postgres project (via the connection pooler, not the direct connection — negrita's Docker containers have no IPv6 route and Supabase's direct connection is IPv6-only), old MariaDB accessory removed, no data migrated (there was none worth carrying over). Storage/Realtime/Search ride on the spine but are separate follow-up work. See [Supabase Migration](supabase-migration.md). Auth and the [MCP server](mcp-server.md) unchanged.
* **Admin (#20, moved up 2026-08-24)** — ✅ backend done (2026-08-30, `feat/admin-audit-logs`): `is_admin` added to `users`; admin mutates the per-user quota columns (no new table). `is_admin` is not stored in the JWT — the API queries `users.is_admin` per admin request, so promote/demote takes effect on the next request. First admin bootstrapped via env `ADMIN_UUID` during JIT provisioning. `GET`/`PATCH /api/admin/users/:id` live and smoke-tested against local Postgres. **Bundled in the same effort**: full audit logging (every org/org-member/board/list/card mutation, plus admin's own quota-override actions, gets an append-only before/after diff row) — new `audit_logs` table, `App\Service\AuditLogService`, `GET /api/admin/audit-logs` (platform-wide) and `GET /api/orgs/:id/audit-logs` (owner-scoped), closing the visibility gap [permissions.md](permissions.md) flagged. See [API Contract](api-contract.md#admin) and [API Contract](api-contract.md#audit-log). Admin UI (`web/` route) not yet built — frontend is the remaining piece.
...
* **Admin feature** (#20) — backend ✅ done (see above); admin UI (`web/` route) still to build.
