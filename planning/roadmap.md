...
* **Search** — across boards/cards. **DECIDED 2026-08-24: Postgres `FULLTEXT` + pgvector**, not MySQL FULLTEXT or Algolia. Gates on the [all-in migration](supabase-migration.md).
...
* **Admin (#20, moved up 2026-08-24)**: add `is_admin` to the existing `users` table; admin mutates the per-user quota columns (no new table). Admin UI is a route in `web/`, not a separate app. `is_admin` is not a JWT flag (Supabase doesn't put `app_metadata` in the access token) — the API queries `users.is_admin` per admin request. First admin bootstrapped via env `ADMIN_UUID`. Minimal version ships now on MySQL `users` (identical column shape survives the migration). Contract TBD pending the implementing branch — see [log.md](log.md) 2026-08-24 (cont.).
