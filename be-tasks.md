...
### A — `feat/api-scaffold-db`

- [ ] `composer create-project cakephp/app` skeleton in `/api`. Pin `"php": "8.5.*"` in `composer.json`. Confirm the
...
- [ ] Wire DB connection to the `docker/` MariaDB service using `DATABASE_URL` in `api/.env.example` (
